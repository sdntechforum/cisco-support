...
class SoftwareApi extends base_api_js_1.BaseApi {
    baseUrl = 'https://apix.cisco.com/software/suggestion/v2';
    apiName = 'Software';
    getTools() {
        return [
            {
    ...
    async getSoftwareSuggestionsByProductIds(args) {
        const params = {};
        if (args.page_index)
            params.pageIndex = args.page_index;
    ...
    async getSoftwareReleasesByProductIds(args) {
        const params = {};
        if (args.page_index)
            params.pageIndex = args.page_index;
    ...
    async getCompatibleSoftwareByProductId(args) {
        const params = {};
        if (args.page_index)
            params.pageIndex = args.page_index;
        if (args.current_image)
            params.currentImage = args.current_image;
        if (args.current_release)
            params.currentRelease = args.current_release;
    ...
    async getSoftwareSuggestionsByMdfIds(args) {
        const params = {};
        if (args.page_index)
            params.pageIndex = args.page_index;
    ...
    async getSoftwareReleasesByMdfIds(args) {
        const params = {};
        if (args.page_index)
            params.pageIndex = args.page_index;
    ...
    async getCompatibleSoftwareByMdfId(args) {
        const params = {};
        if (args.page_index)
            params.pageIndex = args.page_index;
        if (args.current_image)
    ...
    formatSoftwareSuggestionResponse(data, title) {
        ...
        if (data.productList && Array.isArray(data.productList)) {
            const formattedProducts = data.productList.map((productItem) => this.formatProductSuggestion(productItem));
            return {
                data: {
                    title,
        ...
        if (data.suggestions && Array.isArray(data.suggestions)) {
            const formattedSuggestions = data.suggestions.map((suggestion) => this.formatCompatibleSuggestion(suggestion));
            return {
                data: {
                    title,
    ...
    formatProductSuggestion(productItem) {
        ...
        return {
            ...
            suggestions: suggestions.map((suggestion) => this.formatSuggestion(suggestion)),
            // Links
    ...
    formatCompatibleSuggestion(suggestion) {
        return {
            // Basic product identification
    ...
    generateSoftwareDownloadUrl(productId) {
        if (!productId)
            return null;
    ...
    generateCiscoProductUrl(productId) {
        if (!productId)
            return null;
...
//# sourceMappingURL=software-api.js.map