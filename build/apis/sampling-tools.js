...
exports.samplingTools = void 0;
exports.handleSamplingTool = handleSamplingTool;
const sampling_js_1 = require("../utils/sampling.js");
const logger_js_1 = require("../utils/logger.js");
/**
 * Tool definitions for sampling-powered features
 */
exports.samplingTools = [
    {
 ...
 */
async function handleSamplingTool(server, toolName, args, ciscoAuth) {
    // Check if client supports sampling
    if (!(0, sampling_js_1.clientSupportsSampling)(server)) {
        return {
            content: [
                {
                    type: 'text',
...
Use the standard (non-AI) tools which don't require sampling support.`
    ...
    try {
        ...
        logger_js_1.logger.error(`Sampling tool error: ${toolName}`, { error: error.message });
        return {
            content: [
                {
                    type: 'text',
 ...
 */
async function handleResolveProductName(server, args) {
    const { product_description } = args;
    logger_js_1.logger.info('Resolving product name with AI', { product_description });
    const productId = await (0, sampling_js_1.resolveProductName)(server, product_description);
 ...
 */
async function handleCategorizeBug(server, args) {
    const { bug_description } = args;
    logger_js_1.logger.info('Categorizing bug with AI', { descriptionLength: bug_description.length });
    const analysis = await (0, sampling_js_1.categorizeBug)(server, bug_description);
    const severityEmoji = {
        critical: '🔴',
        high: '🟠',
        medium: '🟡',
 ...
 */
async function handleAnalyzeUpgradeRisk(server, args, ciscoAuth) {
    const { product_id, current_version, target_version } = args;
    logger_js_1.logger.info('Analyzing upgrade risk with AI', { product_id, current_version, target_version });
    ...
    const analysis = await (0, sampling_js_1.analyzeUpgradeRisk)(server, product_id, current_version, target_version, bugData);
    return {
        content: [
            {
                type: 'text',
 ...
 */
async function handleSummarizeBugs(server, args, ciscoAuth) {
    const { bug_ids, search_context } = args;
    logger_js_1.logger.info('Summarizing bugs with AI', { bug_ids, search_context });
    // Fetch bug details
    const bugIdArray = bug_ids.split(',').map((id) => id.trim()).slice(0, 10);
    // In production, fetch actual bug data via get_bug_details
    const bugs = bugIdArray.map((id) => ({
        bug_id: id,
        note: 'In production, this would fetch actual bug data'
    }));
    const summary = await (0, sampling_js_1.summarizeBugs)(server, bugs, search_context);
    return {
        content: [
            {
                type: 'text',
 ...
 */
async function handleExtractProductQuery(server, args) {
    const { natural_query } = args;
    logger_js_1.logger.info('Extracting product query with AI', { natural_query });
    const extracted = await (0, sampling_js_1.extractProductQuery)(server, natural_query);
    return {
        content: [
            {
                type: 'text',
...
//# sourceMappingURL=sampling-tools.js.map