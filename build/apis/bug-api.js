...
class BugApi extends base_api_js_1.BaseApi {
        ...
        for (let severity = 1; severity <= maxSeverity; severity++) {
            try {
                ...
                if (result.bugs && Array.isArray(result.bugs)) {
                    ...
                    logger_js_1.logger.info(`Found ${count} bugs at severity ${severity}`);
                }
                else {
            ...
            switch (searchType) {
                case 'keyword':
                    ...
                    if (keywordTerm.length > 50) {
                        ...
                        logger_js_1.logger.info('Shortened keyword search term', {
                ...
                case 'product_id':
                    ...
                    if (!productIdResult.bugs || productIdResult.bugs.length === 0) {
                        logger_js_1.logger.info('Product ID search returned no results, falling back to keyword search', {
                            searchTerm,
    ...
    async executeComprehensiveAnalysis(args, meta) {
        const productIdentifier = args.product_identifier;
        const softwareVersion = args.software_version;
        const analysisFocus = args.analysis_focus || 'comprehensive';
        const includeWebSearchGuidance = args.include_web_search_guidance !== false;
        logger_js_1.logger.info('Starting comprehensive analysis', { productIdentifier, softwareVersion, analysisFocus });
        ...
        productSeries = web_search_js_1.WebSearchHelper.getProductSeries(productIdentifier);
        if (productIdentifier.toLowerCase().includes('cisco') && productIdentifier.length > 20) {
            ...
            logger_js_1.logger.info('Processed product identifier', {
        ...
        try {
            analysis.product_resolution = await web_search_js_1.WebSearchHelper.resolveProductName(productIdentifier);
            analysis.search_strategy_used.push('Product ID resolution via known mappings and patterns');
        }
        catch (error) {
            logger_js_1.logger.error('Product resolution failed', { error });
        }
        ...
        try {
            ...
            if (productSeries && softwareVersion) {
                ...
                try {
                    ...
                    const ciscoFormattedVersion = web_search_js_1.WebSearchHelper.formatVersionForCiscoAPI(softwareVersion);
                    ...
                    if (productSeriesResult.bugs && analysis.bug_analysis.bugs) {
                        ...
                        const uniqueBugs = combinedBugs.filter((bug, index, self) => index === self.findIndex(b => b.bug_id === bug.bug_id));
                        analysis.bug_analysis.bugs = uniqueBugs;
                        analysis.bug_analysis.total_results = uniqueBugs.length;
                        logger_js_1.logger.info('Combined product series search results', {
                    ...
                    logger_js_1.logger.warn('Product series search failed', { productSeries, softwareVersion, error });
                }
            }
            ...
            logger_js_1.logger.error('Bug analysis failed in comprehensive analysis', { error });
        ...
        if (includeWebSearchGuidance) {
            try {
                analysis.web_search_guidance = {
                    lifecycle_queries: web_search_js_1.WebSearchHelper.generateLifecycleSearchQueries(productIdentifier, softwareVersion),
                    product_info: analysis.product_resolution,
                    recommended_searches: []
                };
                // Add context-specific web search guidance
                if (analysisFocus === 'incident_response') {
                    const incidentStrategy = web_search_js_1.WebSearchHelper.generateIncidentSearchStrategy(productIdentifier, softwareVersion);
                ...
                if (softwareVersion) {
                    analysis.web_search_guidance.recommended_searches.push(`"${softwareVersion}" bugs fixes site:cisco.com`, `"${softwareVersion}" known issues site:cisco.com`);
                }
                analysis.search_strategy_used.push('Web search guidance generation for external research');
            }
            catch (error) {
                logger_js_1.logger.error('Web search guidance generation failed', { error });
            }
        }
        ...
        if (analysis.bug_analysis && analysis.bug_analysis.bugs) {
            ...
            const openBugs = analysis.bug_analysis.bugs.filter((b) => b.status === 'O').length;
            const criticalBugs = analysis.bug_analysis.bugs.filter((b) => ['1', '2'].includes(b.severity)).length;
            if (criticalBugs > 0) {
    ...
    async executeCompareSoftwareVersions(args, meta) {
        const productId = args.product_id;
        const versionA = args.version_a;
        const versionB = args.version_b;
        const includeCveAnalysis = args.include_cve_analysis !== false;
        const includeEolStatus = args.include_eol_status !== false;
        const includeRecommendations = args.include_recommendations !== false;
        const maxSeverity = args.max_severity || 3;
        logger_js_1.logger.info('Starting software version comparison', {
        ...
        try {
            ...
            sendProgress(meta?.progressToken, 0, totalSteps);
            logger_js_1.logger.info('Analyzing bugs for both versions');
            ...
            if (includeCveAnalysis) {
                logger_js_1.logger.info('Analyzing CVEs and security advisories');
            ...
            if (includeRecommendations) {
                logger_js_1.logger.info('Getting software recommendations');
            ...
            sendProgress(meta?.progressToken, 3, totalSteps);
            if (includeEolStatus) {
                logger_js_1.logger.info('Checking end-of-life status');
            ...
            logger_js_1.logger.info('Software version comparison completed successfully', {
                productId,
                versionA,
                versionB,
            ...
            logger_js_1.logger.error('Error during software version comparison', {
    ...
    async executeProductNameResolver(args) {
        const productId = args.product_id;
        const includeSearchStrategies = args.include_search_strategies !== false;
        logger_js_1.logger.info('Resolving product name', { productId });
        try {
            const resolution = await web_search_js_1.WebSearchHelper.resolveProductName(productId);
            const result = {
                product_id: productId,
                resolution: resolution,
                search_strategies: includeSearchStrategies ? {
                    lifecycle_queries: web_search_js_1.WebSearchHelper.generateLifecycleSearchQueries(productId),
                    general_queries: [
            ...
            logger_js_1.logger.error('Product name resolution failed', { productId, error });
            return {
                bugs: [],
                total_results: 0,
                page_index: 1,
    ...
    async compareBugsBetweenVersions(productId, versionA, versionB, maxSeverity) {
        try {
            ...
            // Analyze differences
            const versionABugIds = new Set(bugsA.bugs?.map((bug) => bug.bug_id) || []);
            const versionBBugIds = new Set(bugsB.bugs?.map((bug) => bug.bug_id) || []);
            // Find shared bugs
            ...
            return {
                ...
                shared_bugs: (bugsA.bugs || []).filter((bug) => sharedBugIds.has(bug.bug_id)),
                fixed_in_version_b: (bugsA.bugs || []).filter((bug) => fixedInBIds.has(bug.bug_id)),
                introduced_in_version_b: (bugsB.bugs || []).filter((bug) => introducedInBIds.has(bug.bug_id)),
                analysis: {
            ...
            logger_js_1.logger.error('Bug comparison failed', { productId, versionA, versionB, error });
            return {
    ...
    async searchBugsForVersion(productId, version, maxSeverity) {
    ...
    async analyzeCvesBetweenVersions(productId, versionA, versionB) {
    ...
    async getSoftwareRecommendations(productId, versionA, versionB) {
    ...
    async checkEolStatus(versionA, versionB) {
    ...
    generateVersionComparisonRecommendations(comparison) {
    ...
    determineOverallRecommendation(comparison) {
        const summary = comparison.summary;
        const netImprovement = summary.fixed_in_b - summary.introduced_in_b;
        if (netImprovement > 5) {
...
exports.BugApi = BugApi;
//# sourceMappingURL=bug-api.js.map