...
class ProductApi extends base_api_js_1.BaseApi {
    baseUrl = 'https://apix.cisco.com/product/v1/information';
    apiName = 'Product';
    getTools() {
        return [
            {
    ...
    async getProductInfoBySerialNumbers(args) {
        // Validate serial numbers count
        const serialNumbers = args.serial_numbers.split(',').map((s) => s.trim());
        ...
        const params = {};
        if (args.page_index)
            params.page_index = args.page_index;
    ...
    async getProductInfoByProductIds(args) {
        // Validate product IDs count
        const productIds = args.product_ids.split(',').map((s) => s.trim());
    ...
    async getProductMdfInfoByProductIds(args) {
        // Validate product IDs count
        const productIds = args.product_ids.split(',').map((s) => s.trim());
        if (productIds.length > 5) {
            return {
    ...
    formatProductInfoResponse(data, title) {
    ...
    formatProductMdfResponse(data, title) {
        ...
        const formattedProducts = products.map((product) => this.formatMdfProduct(product));
        return {
            data: {
                title,
                count: products.length,
