...
exports.WebSearchHelper = void 0;
...
class WebSearchHelper {
     ...
     */
    static async resolveProductName(productId) {
        const result = {
            fullName: null,
            modelUrl: null,
            searchStrategy: []
        ...
        try {
            logger_js_1.logger.info('Attempting to resolve product name via web search', { productId });
            ...
            if (mapping) {
                ...
                logger_js_1.logger.info('Product resolved via known mappings', { productId, fullName: result.fullName });
                return result;
            }
            ...
            if (productId.match(/^ISR44\d+/i)) {
                ...
                logger_js_1.logger.info('ISR product resolved via pattern matching', { productId, fullName: result.fullName });
                return result;
            }
            ...
            if (productId.match(/^(WS-)?C\d+/i)) {
                ...
                logger_js_1.logger.info('Catalyst product resolved via pattern matching', { productId, fullName: result.fullName });
                return result;
            }
            ...
            if (productId.match(/^ASR\d+/i)) {
                ...
                logger_js_1.logger.info('ASR product resolved via pattern matching', { productId, fullName: result.fullName });
                return result;
            }
            logger_js_1.logger.warn('Could not resolve product name', { productId });
            return result;
        }
        catch (error) {
            logger_js_1.logger.error('Web search resolution failed', {
                productId,
     ...
     */
    static generateLifecycleSearchQueries(productId, softwareVersion) {
        const queries = [];
     ...
     */
    static generateIncidentSearchStrategy(productId, softwareVersion, symptom) {
     ...
     */
    static getKnownProductMappings() {
        return {
            'ISR4431/K9': {
...
exports.WebSearchHelper = WebSearchHelper;
