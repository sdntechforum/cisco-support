...
exports.authenticateWithSmartBonding = authenticateWithSmartBonding;
exports.getValidSmartBondingToken = getValidSmartBondingToken;
exports.resetSmartBondingToken = resetSmartBondingToken;
exports.getSmartBondingTokenStatus = getSmartBondingTokenStatus;
const logger_js_1 = require("./logger.js");
// Global variables for Smart Bonding OAuth2 token management
let sbAccessToken = null;
let sbTokenExpiry = null;
 ...
 */
async function authenticateWithSmartBonding() {
    ...
    try {
        logger_js_1.logger.info('Requesting Smart Bonding OAuth2 token from Cisco Cloud SSO');
        ...
        if (!response.ok) {
            ...
            logger_js_1.logger.error('Smart Bonding OAuth2 authentication failed', {
        ...
        logger_js_1.logger.info('Successfully obtained Smart Bonding OAuth2 token', {
            expiresIn: expiresIn,
        ...
        if (error instanceof Error) {
            if (error.name === 'AbortError' || error.message.includes('timeout')) {
                logger_js_1.logger.error('Smart Bonding OAuth2 authentication timed out', { timeout: '30s' });
                ...
                logger_js_1.logger.error('Smart Bonding OAuth2 headers timeout');
                throw new Error(`Smart Bonding OAuth2 authentication connection timed out. Cisco Cloud SSO may be temporarily unavailable.`);
            }
        }
        logger_js_1.logger.error('Smart Bonding OAuth2 authentication failed', error);
        throw error;
    }
}
/**
 ...
 */
async function getValidSmartBondingToken() {
    const now = Date.now();
    ...
    if (sbAccessToken && sbTokenExpiry && (sbTokenExpiry - now) > SB_TOKEN_REFRESH_MARGIN) {
        logger_js_1.logger.info('Using cached Smart Bonding token', {
            timeRemaining: Math.floor((sbTokenExpiry - now) / 1000 / 60) + ' minutes'
        });
        return sbAccessToken;
    }
    logger_js_1.logger.info('Smart Bonding token expired or missing, refreshing...', {
 ...
 */
function resetSmartBondingToken() {
    sbAccessToken = null;
    sbTokenExpiry = null;
    logger_js_1.logger.info('Smart Bonding token reset');
}
/**
 * Get current token status for debugging
 */
function getSmartBondingTokenStatus() {
...
//# sourceMappingURL=smart-bonding-auth.js.map