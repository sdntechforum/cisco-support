...
const toon_formatter_js_1 = require("./utils/toon-formatter.js");
...
function formatResults(result, apiName, toolName, args) {
    ...
    const useToon = (0, toon_formatter_js_1.shouldUseToonFormat)();
    const formatType = (0, toon_formatter_js_1.getFormatDescription)();
    logger_js_1.logger.info(`Formatting results as ${formatType}`, { apiName, toolName });
    // If TOON format is enabled, use TOON formatting for JSON data
    if (useToon) {
        try {
            if (apiName === 'Bug' || toolName.includes('bug')) {
                return (0, toon_formatter_js_1.bugResponseToToon)(result);
            }
            else if (apiName === 'Case' || toolName.includes('case')) {
                return (0, toon_formatter_js_1.caseResponseToToon)(result);
            }
            else if (apiName === 'EoX' || toolName.includes('eox')) {
                return (0, toon_formatter_js_1.eoxResponseToToon)(result);
            }
            else {
            ...
            logger_js_1.logger.warn('TOON formatting failed, falling back to standard format', { error });
...
`;
                ...
                if (!webCookie) {
                    ...
                    try {
                        const response = await fetch(`https://bst.cloudapps.cisco.com/api/productAutocomplete?productsearchTerm=${encodeURIComponent(searchTerm)}`, {
                            headers: {
                                'Cookie': webCookie,
