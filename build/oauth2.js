...
exports.handleAuthorizeRequest = handleAuthorizeRequest;
exports.handleAuthorizeApproval = handleAuthorizeApproval;
...
exports.scopesToEnabledAPIs = scopesToEnabledAPIs;
...
const refreshTokens = new Map(); // refresh_token -> access_token mapping
// Configuration
...
function loadClientSecrets() {
    const secretsPath = process.env.OAUTH_SECRETS_CONFIG || (0, node_path_1.resolve)(process.cwd(), 'config/oauth-secrets.json');
    const secrets = new Map();
    if (!(0, node_fs_1.existsSync)(secretsPath)) {
    ...
    try {
        const secretsContent = (0, node_fs_1.readFileSync)(secretsPath, 'utf-8');
 ...
 */
function loadClientsFromConfig() {
    const configPath = process.env.OAUTH_CLIENTS_CONFIG || (0, node_path_1.resolve)(process.cwd(), 'config/oauth-clients.json');
 ...
 */
function watchClientsConfig() {
    const configPath = process.env.OAUTH_CLIENTS_CONFIG || (0, node_path_1.resolve)(process.cwd(), 'config/oauth-clients.json');
    if (!(0, node_fs_1.existsSync)(configPath)) {
        return;
    }
    (0, node_fs_1.watchFile)(configPath, { interval: 2000 }, () => {
 ...
 */
function generateAuthorizationServerMetadata(config) {
    return {
 ...
 */
function validateScopes(requestedScope, clientScopes) {
...
function validatePKCE(codeVerifier, codeChallenge, codeChallengeMethod) {
    ...
    if (codeChallengeMethod === 'S256') {
        const hash = (0, node_crypto_2.createHash)('sha256').update(codeVerifier).digest('base64url');
 ...
 */
function isSecureRedirectUri(uri) {
    try {
 ...
 */
async function fetchClientMetadata(clientIdUrl) {
    try {
...
function registerClient(req, res, config) {
    try {
        ...
        const { redirect_uris, grant_types = ['authorization_code'], response_types = ['code'], client_name, client_uri } = req.body;
        ...
        const client_id = `mcp_${(0, node_crypto_1.randomUUID)().replace(/-/g, '')}`;
        const client_secret = (0, node_crypto_1.randomUUID)().replace(/-/g, '');
        const client = {
            client_id,
            client_secret,
            redirect_uris,
...
async function handleAuthorizeRequest(req, res) {
    try {
        const { response_type, client_id, redirect_uri, code_challenge, code_challenge_method = 'plain', scope, state } = req.query;
        ...
        const validatedScope = validateScopes(scope, client.scopes || ['mcp']);
...
function handleTokenRequest(req, res) {
    try {
        let { grant_type, code, redirect_uri, client_id, client_secret, code_verifier, refresh_token } = req.body;
        ...
        if (grant_type === 'authorization_code') {
            ...
            const access_token = (0, node_crypto_1.randomUUID)().replace(/-/g, '');
            const refresh_token_value = (0, node_crypto_1.randomUUID)().replace(/-/g, '');
            const token = {
                access_token,
                token_type: 'Bearer',
            ...
            const new_access_token = (0, node_crypto_1.randomUUID)().replace(/-/g, '');
            const new_refresh_token = (0, node_crypto_1.randomUUID)().replace(/-/g, '');
            const newToken = {
 ...
 */
function createOAuth2Middleware() {
    return (req, res, next) => {
 ...
 */
function scopesToEnabledAPIs(scopes) {
